...
function extractEmail(claims) {
    if (claims.email)
        return claims.email;
    const identities = claims.raw.firebase
        ?.identities;
    if (identities) {
...
async function attachUserFromToken(req, idToken) {
...
export async function authMiddleware(req, _res, next) {
    if (!firebaseConfigured) {
        return next(new UnauthorizedError('Firebase project id is not configured on the server', 'AUTH_NOT_CONFIGURED'));
    }
...
export async function optionalAuthMiddleware(req, _res, next) {
    const authHeader = req.headers.authorization;
    if (!authHeader?.startsWith('Bearer ') || !firebaseConfigured)
        return next();
    try {
        await attachUserFromToken(req, authHeader.slice(7));
    }
